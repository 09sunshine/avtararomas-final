...
import { validateCoupon, fetchStoreSettings, type ApiStoreSettings } from "../lib/api";
...
export default function Cart() {
  ...
  const [settings, setSettings] = useState<ApiStoreSettings>({
  ...
  const discountAmount = state.coupon ? Math.round((subtotal * state.coupon.discountPercent) / 100) : 0;
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (res.valid && res.discountPercent !== undefined && res.code) {
        const discountAmt = res.discountAmount ?? Math.round((subtotal * res.discountPercent) / 100);
        dispatch({
          type: "SET_COUPON",
          coupon: {
            code: res.code,
                ...
                >
                  <Link to={`/shop/${product.slug}`} className="flex-shrink-0">
                    <div className="w-20 h-28 sm:w-24 sm:h-32 overflow-hidden bg-[#0A0908]">
                      <img src={normalizeImageUrl(product.images[0])} alt={product.name} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                    </div>
                  </Link>
                  ...
                  <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-3">
                      <div className="flex items-center border border-[rgba(201,169,110,0.2)]">
                        <button
                          onClick={() => dispatch({ type: "UPDATE_QTY", productId: product.id, quantity: quantity - 1, size })}
          ...
          <div className="sticky top-28 self-start">
            <div className="bg-[#0A0908] border border-[rgba(201,169,110,0.1)] p-8">
              ...
              <div className="mb-7">
                <div className="flex gap-0">
                  ...
                  <button
                    ...
                    className="px-5 py-3 border border-[rgba(201,169,110,0.2)] text-xs text-primary hover:bg-primary/10 transition-colors disabled:opacity-50"
                ...
                {state.coupon && (
                  <div className="flex items-center justify-between text-xs text-primary mt-2" style={{ fontFamily: "var(--font-mono)", fontWeight: 300 }}>
                    <span>✓ {state.coupon.code} — {state.coupon.discountPercent}% off applied</span>
                    <button
                      onClick={() => { dispatch({ type: "SET_COUPON", coupon: null }); setCouponCode(""); }}
                      className="text-[10px] text-muted-foreground hover:text-red-400 underline ml-2 cursor-pointer"
                    >
                      Remove
                    </button>
                  </div>
              ...
              <div className="flex flex-col gap-3.5 border-t border-[rgba(201,169,110,0.08)] pt-6">
                ...
                {discountAmount > 0 && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground" style={{ fontFamily: "var(--font-body)", fontWeight: 300 }}>Discount ({state.coupon?.discountPercent}%)</span>
